...
export default class MochaJSDelegate {
  constructor(selectorHandlerDict, superclass) {
    ...
    this.delegateClassDesc = MOClassDescription.allocateDescriptionForClassWithName_superclass_(
      this.uniqueClassName,
      superclass || NSObject,
    );
  ...
  setHandlerForSelector(selectorString, func) {
    ...
    */
    if (!handlerHasBeenSet) {
      ...
      const code = NSString.stringWithFormat(
        '(function (%@) %@)',
        args.join(', '),
        body,
      );
      ...
      this.delegateClassDesc.addInstanceMethodWithSelector_function_(
        selector,
        dynamicFunction,
      );
    }
